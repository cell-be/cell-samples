...
namespace Nes
{
	...
	class queword
	{
		void Multiply(queword);
		static void Divide(queword&,const queword,bool);
		void Shl(uint);
		void Shr(uint);
	...
	public:
		...
		queword(dword msdw,dword lsdw)
		: lo(lsdw), hi(msdw) {}

		queword(const queword& v)
		: lo(v.lo), hi(v.hi) {}

		template<typename V>
		queword& operator = (const V& v)
		{
			lo = v;
			hi = 0;
			return *this;
		...
		queword& operator = (const queword& v)
		{
			lo = v.lo;
			hi = v.hi;
			return *this;
		...
		queword& operator %= (const V& v)
		{
			if (!hi)
				...
				Divide( *this, queword(v), true );

			return *this;
		}

		template<typename V> queword operator + (const V& v) const { return queword(*this) += v; }
		template<typename V> queword operator - (const V& v) const { return queword(*this) -= v; }
		template<typename V> queword operator * (const V& v) const { return queword(*this) *= v; }
		template<typename V> queword operator / (const V& v) const { return queword(*this) /= v; }
		template<typename V> queword operator % (const V& v) const { return queword(*this) %= v; }

		template<typename V> queword& operator |= (const V& v) { lo |= v;         return *this; }
		template<typename V> queword& operator &= (const V& v) { lo &= v; hi = 0; return *this; }
		template<typename V> queword& operator ^= (const V& v) { lo ^= v;         return *this; }

		template<typename V> queword operator | (const V& v) const { return queword( hi, lo | v ); }
		template<typename V> queword operator & (const V& v) const { return queword(     lo & v ); }
		template<typename V> queword operator ^ (const V& v) const { return queword( hi, lo ^ v ); }

		template<typename V> queword& operator >>= (const V& v) { Shr(v); return *this; }
		template<typename V> queword& operator <<= (const V& v) { Shl(v); return *this; }

		template<typename V> queword operator >> (const V& v) const { return queword(*this) >>= v; }
		template<typename V> queword operator << (const V& v) const { return queword(*this) <<= v; }

		queword operator ~() const
		{
			return queword( hi ^ LO_MASK, lo ^ LO_MASK );
		}
	...
	template<>
	inline queword& queword::operator += (const queword& v)
	{
		dword t = lo;
		lo = (lo + v.lo) & LO_MASK;
	...
	template<> inline queword& queword::operator |= (const queword& v) { lo |= v.lo; hi |= v.hi; return *this; }
	template<> inline queword& queword::operator &= (const queword& v) { lo &= v.lo; hi &= v.hi; return *this; }
	template<> inline queword& queword::operator ^= (const queword& v) { lo ^= v.lo; hi ^= v.hi; return *this; }

	template<> inline queword queword::operator | (const queword& v) const { return queword( hi | v.hi, lo | v.lo ); }
	template<> inline queword queword::operator & (const queword& v) const { return queword( hi & v.hi, lo & v.lo ); }
	template<> inline queword queword::operator ^ (const queword& v) const { return queword( hi ^ v.hi, lo ^ v.lo ); }

	template<>
	inline bool queword::operator == (const queword& v) const
	{
